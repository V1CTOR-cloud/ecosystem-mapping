...
function LocationComponent(props) {
    ...
    useEffect(() => {
        const tempLocation = {
            continent: "Continent", country: "Country", region: "Region", city: "City",
        };
    ...
    useEffect(() => {
        ...
        props.locations.forEach((country) => {
            const containsContinent = tempContinentList.some((continent) => continent.name === country.continent);

            // Create the list of continent
            if (!containsContinent) {
                tempContinentList.push({
                    id: tempContinentList.length, name: country.continent,
                });
            }
        });
    ...
    useEffect(() => {
        ...
        props.locations.forEach((country) => {
            if (country.continent === continent) {
                const containsCountry = tempCountryList.some((country) => country.name === country.country);

                // Create the list of country
                if (!containsCountry) {
                    tempCountryList.push({
                        id: tempCountryList.length, name: country.country, regions: country.regions,
                    });
                }
            }
        });
    ...
    useEffect(() => {
        if (countryList.length !== 1) {
            ...
            const thisCountry = countryList.find((thisCountry) => thisCountry.name === country);

            if (thisCountry.regions) {
                thisCountry.regions.forEach((region) => {
                    const containsRegion = tempRegionList.some((country) => country.name === country.country);

                    // Add to the list of region
                    if (!containsRegion) {
                        tempRegionList.push({
                            id: tempRegionList.length, name: region.name, cities: region.cities,
                        });
                    }
                });
            }
    ...
    useEffect(() => {
        if (regionList.length !== 1) {
            ...
            const thisRegion = regionList.find((thisRegion) => thisRegion.name === region);

            // Add to the list of cities
            if (thisRegion.cities) {
                thisRegion.cities.forEach((city) => {
                    const containsCity = tempCityList.some((thisCity) => city.name === thisCity.name);

                    if (!containsCity) {
                        tempCityList.push({
                            id: tempCityList.length, name: city.name,
                        });
                    }
                });
            }
    ...
    return <>
        ...
        <HStack justifyContent="space-between">
            ...
            <SmallLabelLocation
                ...
                wantScroll={true}
                isDisabled={location.country === countryList[0].name || regionList.length === 1}
                items={regionList}
            ...
            <SmallLabelLocation
                label={t("common.city")}
                wantScroll={true}
                isDisabled={location.region === regionList[0].name || cityList.length === 1}
                items={cityList}
                item={location.city}
