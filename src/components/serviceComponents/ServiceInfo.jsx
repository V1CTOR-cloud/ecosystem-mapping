...
const ServiceInfo = ({ ServiceInfoData, dataS = { serviceName: "" }, isEdit }) => {
  ...
  return (
    <ModalBody pb={6}>
      <FormControl>
        ...
        {/* <Input ref={initialRef} className="fm-ip-flds" /> */}
        <Select 
      ...
      <FormControl mt={4}>
        ...
        {/* <Input className="fm-ip-flds" /> */}
        <Select value={service2} onChange={(data) => setService2(data.target.value)} className="fm-ip-flds" placeholder={t('startup.popup.service.content.service.select')}>
          {items.map((service) => (
            <option text={service.id}>{service.serviceName}</option>
          ))}
        </Select>
      ...
      <FormControl mt={4}>
        ...
        />
        <FormControl mt={4}>
          <FormLabel className="frm-lbl">{t('startup.popup.service.content.service.choose.audience')}</FormLabel>
          <Multiselect
            className="fm-ip-flds"
            placeholder={t('startup.popup.service.content.service.select.audience')}
            showCheckbox
            value={audience}
            options={Audience}
            isObject={false}
            ...
            onSelect={ (selectedAudienceList) => setAudience(selectedAudienceList)}
          />
        </FormControl>{" "}
        <FormControl mt={4}>
          <FormLabel className="frm-lbl">{t('startup.popup.service.details.content.budget')}</FormLabel>
          {/* <Input className="fm-ip-flds" /> */}
          <Select
            className="fm-ip-flds"
            placeholder={t('startup.popup.service.content.service.select.budget')}
            value={budget}
      ...
      <FormControl mt={4}>
        <FormLabel className="frm-lbl">{t('startup.popup.service.content.service.verified')}</FormLabel>
        <HStack spacing={10} direction="row">
          <RadioGroup onChange={setVerifiedDetails} value={verifiedDetails}>
            <Stack direction="row">
              <Radio value="1">{t('startup.popup.service.content.service.yes')}</Radio>
              <Box w="10px" />
              <Radio value="2">{t('startup.popup.service.content.service.no')}</Radio>
            </Stack>
          </RadioGroup>
        </HStack>
      </FormControl>
