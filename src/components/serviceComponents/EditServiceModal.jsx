...
const EditServiceModal = ({ data , succesClose}) => {
  ...
  useEffect(() => {
    setBasicInfo([ data.serviceName, data.serviceOwner[0].id, data.serviceFocus, data.applicationType])
    setServiceInfo([ data.previousService, data.followingService, data.serviceBreif, data.serviceDescription, data.serviceOutcomes, data.serviceAudience, data.budget ])
    setServiceAvailability([ data.serviceStartTime, data.serviceEndTime, data.timezone, data.serviceLocation ])
          ...
          <Accordion defaultIndex={[0]} allowMultiple>
            ...
            <AccordionItem mt="5" border="0px" boxShadow="md">
              ...
              <AccordionPanel pb={4} className="acc-panel">
                <ServiceAvailability
                  dataS={data}
                  ServiceAvailabilityData={useCallback((a, b, c, d, e, f, g, h) => {
